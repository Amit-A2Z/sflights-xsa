...
> *"Show me all airlines with more than 80% seat occupancy"*
...
|----------|-------------|
| *"List all airlines sorted by name"* | Claude calls the `carriers` resource with `$orderby=CARRNAME` |
| *"How many bookings in Q1 2025?"* | Claude queries `bookings` with date filter |
| *"Show flights to New York with occupancy > 80%"* | Claude uses `flights` resource with compound filter |
| *"Which airline has the most passengers?"* | Claude may call multiple resources and aggregate |
