...
```bash
git clone https://github.com/AKS91/sflights-xsa.git
cd sflights-xsa
npm install
cds watch
```
...
```json
{
  "mcpServers": {
    "cap-tools": {
      "command": "node",
      "args": ["<FULL-PATH-TO>/sflights-xsa/mcp-server/build/index.js"]
    }
  }
}
```
...
> Example on Mac: `/Users/yourname/projects/sflights-xsa/mcp-server/build/index.js`
> Example on Windows: `C:/Users/yourname/projects/sflights-xsa/mcp-server/build/index.js`
...
This creates a file like `mta_archives/sflights-cap_1.0.0.mtar`.
...
- Assign `sflights-cap-admin` or `sflights-cap-viewer` to your user

**5. Access your service**

```bash
cf apps
# Note the URL for sflights-cap-approuter
...
cds bind --to sflights-cap-db:sflights-cap-db
...
**Three MTA Modules:**
1. **sflights-cap-approuter** — Handles authentication via XSUAA, routes requests to the service
2. **sflights-cap-srv** — Node.js CAP service exposing OData V4 APIs
3. **sflights-cap-db-deployer** — Deploys CDS model to HANA Cloud HDI container

**Two BTP Services:**
- **XSUAA** — Authentication and authorization (role collections: `sflights-cap-admin`, `sflights-cap-viewer`)
...
This project was transformed from [sflights-xsa](https://github.com/AKS91/sflights-xsa) — originally a HANA XSA HDI-only project with `.hdbcds` tables and `.hdbtabledata` imports. The transformation modernized it to SAP CAP CDS with full OData V4 services, BTP deployment, and AI-powered data exploration through MCP.
