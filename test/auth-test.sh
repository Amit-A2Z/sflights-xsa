...
#   - BTP apps running (cf start sflights-cap-srv sflights-cap-approuter)
...
if [ ! -f "$ENV_FILE" ]; then
  ...
  echo "  Run: cf env sflights-cap-srv > default-env.json"
  echo "  Or:  cds bind --to sflights-cap-db:sflights-cap-db"
  exit 1
fi
...
APPROUTER_URL="https://0ef2ec95trial-dev-sflights-cap-approuter.cfapps.us10-001.hana.ondemand.com"
SRV_URL="https://0ef2ec95trial-dev-sflights-cap-srv.cfapps.us10-001.hana.ondemand.com"
...
if [ -z "$TOKEN" ]; then
  ...
  echo "    - Are the XSUAA credentials current? Re-run: cf env sflights-cap-srv"
...
if [ "$HTTP_CODE" = "200" ]; then
  ...
  echo "  ✗ Approuter: HTTP $HTTP_CODE"
  echo "    Is the approuter running? Check: cf app sflights-cap-approuter"
fi
echo ""
...
if [ "$HTTP_CODE" = "200" ]; then
  ...
  echo "    Is the srv running? Check: cf app sflights-cap-srv"
fi
echo ""
...
echo "   # Start apps:"
echo "   cf start sflights-cap-srv"
echo "   cf start sflights-cap-approuter"
echo ""
